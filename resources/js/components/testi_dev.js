        ...
        let items = this.items.map(function (item, index) {
            ...
            return (<div className="form-group">
                ...
                <input id={item.Field} type={input_type} className="form-control" name={item.Field}
                       placeholder={placeholder} ref={item.Field}/>
            </div>);
        }, this);
        let neat_db_arr = this.db.split("_");
        let neat_db_name = "";
        if (neat_db_arr !== this.db) {
                    ...
                    <div className="row justify-content-center">
                        <div className="col-md-8">
                            <div className="card">
								...
								<h5>Valitse taulu johon lisätään tietoja</h5>
								{radiobuttons}
                                    <FormApp key={this.childKey} onFormSubmit={this.addItem} items={this.items} db={this.db}/>
                                </div>
                            </div>
                        </div>
                    </div>
